...
import logging
from Lessons.Lesson6_Poltoranin.chat.decos import log
        ...
        if 'server.py' in sys.argv and 'client.py' in sys.argv:
                ...
                try:
                    if '-p' in sys.argv:
                        ...
                        if port < 1024 or port > 65535:
                            SERVER_LOGGER.critical(f'Попытка подключение к серверу с указанием неподходящего порта '
            ...
            if 'client.py' in sys.argv:  # client.py 192.168.1.2 8079
                try:
                    ...
                    port = int(sys.argv[2])
                    if port < 1024 or port > 65535:
                        CLIENT_LOGGER.critical(f'Попытка подключение к серверу с указанием неподходящего порта '
                                               f'{port}. Допустимы адреса с 1024 до 65535.')
                        raise ValueError
                except IndexError:
                    address = DEFAULT_ADDRESS
